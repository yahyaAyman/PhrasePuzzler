...
from puzzler_functions import DATA_FILE, PLAYER_ONE, CONSONANT, \
...
def make_guessed(consonants: str, vowels: str, letter: str) -> Tuple[str]:
    """Return a tuple containing consonants and vowels
    with letter removed from them.
...
def finalize_score(puzzle: str, view: str, unguessed_consonants: str, \
    ...
    points to the current_score for each item from unguessed_consonants in
    puzzle that appears as HIDDEN in view.
    ...
    for letter in unguessed_consonants:
        if pf.bonus_letter(puzzle, view, letter):
            final_score = final_score + CONSONANT_BONUS
    return final_score
...
def guess_letter(unguessed_consonants: str, difficulty_level: str) -> str:
    """Return a letter from unguessed_consonants (this function will never return
    a vowel). If difficulty is EASY, the letter is randomly selected.  If
    difficulty is HARD, the letter is the first one from
    PRIORITY_CONSONANTS that occurs in unguessed_letters.

    Precondition: there is a consontant to return.
...
def is_match(puzzle: str, view: str) -> bool:
    """Return True if and only if view could be the puzzle view of puzzle.
    ...
    #  this will no longer give us false positives when we have a character
    #  hidden that was also revealed
...
def get_player_letter(unguessed_consonants: str, unguessed_vowels: str, \
    move_type: str, difficulty: str) -> str:
...
def get_computer_letter(unguessed_consonants: str, unguessed_vowels: str, \
    move_type: str, difficulty: str) -> str:
...
def get_player_move(current_player: int, view: str, difficulty: str, \
...
def get_computer_move(current_player: int, view: str, difficulty: str, \
    score: int, unguessed_consonants: str, unguessed_vowels: str) -> str:
...
def get_player_guess(view: str) -> str:
    ...
    while not pf.game_over(puzzle, view, move_type):
        ...
        if move_type == SOLVE:
            ...
            if get_guess(view) == puzzle:
                ...
                print("The guess '{0}' is Incorrect :-(".format(get_guess(view)))
